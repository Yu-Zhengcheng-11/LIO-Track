...
class Detection {
 public:
    ...
    using Vec3T = Eigen::Matrix<double,3,1>;
 ...
 public:
    ...
    const double errorDynamic(const gtsam::Pose3 x) const;
    const double errorStatic(const gtsam::Pose3 x, jsk_recognition_msgs::BoundingBox objectDetection) const;
};

std::tuple<size_t, double>
getDynamicDetectionIndexAndError(const gtsam::Pose3 &d,
						         std::vector<Detection> detections);
std::tuple<size_t, double>
getStaticDetectionIndexAndError(const gtsam::Pose3 &d,
...
class TightlyCoupledDetectionFactor : public gtsam::NoiseModelFactor2<gtsam::Pose3, gtsam::Pose3> {
 private:
 ...
 protected:
    std::vector<Detection> detections;
	std::vector<gtsam::noiseModel::Diagonal::shared_ptr> noiseModels; // 噪声模型 Σ, 因子的代价函数可以写成 e^{T}*Σ^{-1}*e
    jsk_recognition_msgs::BoundingBox objectDetection;
    gtsam::Pose3 objectVelocity;
 ...
 public:
	...
	TightlyCoupledDetectionFactor(gtsam::Key robotPoseKey,
                                  ...
                                  gtsam::Pose3 objectVelocity,
								  std::vector<Detection> detections)
			: Base(boost::shared_ptr<gtsam::noiseModel::Diagonal>(), robotPoseKey, objectPoseKey),
              objectDetection(objectDetection), objectVelocity(objectVelocity),
    ...
    const jsk_recognition_msgs::BoundingBox getObjectDetection() const { return this->objectDetection; }
    const gtsam::Pose3 getObjectVelocity() const { return this->objectVelocity; }
	...
	virtual size_t dim() const override { return 6; } // 误差向量的维度
};
...
class StablePoseFactor : public gtsam::NoiseModelFactor3<gtsam::Pose3, gtsam::Pose3, gtsam::Pose3> {
 private:
